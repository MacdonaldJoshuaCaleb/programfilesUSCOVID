function [] = USStateFitsFixInts
warning off;
set(0,'DefaultFigureVisible','off')
% import data
paramfits = readtable('paramfitsFixInts.csv');
...
for its = 1:length(Names)
    ...
    betag=3/Tg;
    lsqwt = .05;
    lb = [2/T, 100, .001, .01, .001,0.050,.25*max(NewTests),1.00000,0];
    ub = [6/T,inf,.0995,1.00,.05,.6,max(NewTests),1000,inf];
    ...
    paramguess = paramfits(its,1:9);
    paramguess(7) = .5*max(NewTests);
    temp = paramguess
    fprintf('ITERATION %i OF %i\n',its,length(Names))
    tD = linspace(0,length(Cases)-1,length(Cases));
    [paramfit,resnorm] = lsqcurvefit(@objFxn,paramguess,tD(1:SHE+22),[lsqwt*Cases(1:SHE)',Deaths(1:SHE+22)'],lb,ub);
    %[paramfit,resnorm] = lsqcurvefit(@objFxn,paramfit,tD(1:SHE+30),[lsqwt*Cases(1:SHE)',Deaths(1:SHE+30)'],lb,ub);
    paramfit = [paramfit,22/paramfit(end)];
...
figure
hold on
plot(tD(1:SHE+22),Deaths(1:SHE+22),'r*',tD(1:SHE+22),Dfit,'b','linewidth',2)
if TF ~= 1
h=fill([days(StayHomeStart-FirstCase)-1,days(StayHomeEnd-FirstCase)-1,days(StayHomeEnd-FirstCase)-1,days(StayHomeStart-FirstCase)-1],[0,0,max(Deaths(SHE+22),Dfit(SHE+22)),max(Deaths(SHE+22),Dfit(SHE+22))],'k','LineStyle','none');
    h.FaceAlpha=0.1;
end
ylim([0, max(Deaths(SHE+22),Dfit(SHE+22))])
xlim([0,SHE+22])
...
fname = 'C:\Users\macdo\OneDrive\Desktop\TestingFit\PlotsFixT';
saveas(gca, fullfile(fname, baseFileName), 'png');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure
hold on
plot(tD(1:SHE+22),gampdf(tD(1:SHE+22),paramfit(9),paramfit(10)),'b','linewidth',2)
...
figure
hold on
plot(tD(1:SHE+22),gamcdf(tD(1:SHE+22),paramfit(9),paramfit(10)),'b','linewidth',2)
xlabel('Time Until Death')
...
fig = figure;
   ...
   yyaxis right
   hold on
   plot(tP,tests(tP+2),'r--','linewidth',1)   
   plot(tP,NewTests(tP+2),'r*')
   hold off
...
paramfitsFinalFixInts(its,:) = paramfit;
...
save('paramfitsFinalFixInts.mat','paramfitsFinalFixInts')
