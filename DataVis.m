...
figure
scatter(log(2)./alphas,psis,30,Mortality,'filled') 
...
%ax.XDir = 'reverse';
%view(-31,14)
ylabel('lockdown factor')
xlabel('fatigue half-life, log(2)/\alpha (days)')
xlim([min(log(2)./alphas),max(log(2)./alphas)])
colormap(jet(16))
cb = colorbar;                                     % create and label the colorbar
cb.Label.String = 'Mortality (% population)';
set(gca,'FontSize',16)
...
figure
hold on
scatter(R0s,psis,30,log(2)./alphas,'filled')    % draw the scatter plot
hold off
...
xlabel('R_0')
ylabel('lockdown factor')
colormap(fliplr(jet(16)))
xlim([min(R0s),max(R0s)])
cb = colorbar;                                     % create and label the colorbar
cb.Label.String = 'fatige half-life (days)';
set(gca,'FontSize',16)
figure
hold on
...
xlim([min(R0s),max(R0s)])
cb = colorbar;                                     % create and label the colorbar
cb.Label.String = 'Cum. Case Estimate (% pop.)';
set(gca,'FontSize',16)
figure
hold on

scatter(log(2)./alphas,psis,30,F0BS,'filled')
hold off
...
scatter(log(2)./alphas,psis,30,R0s,'filled')
hold off
...
xlabel('Days Since First Rep. Case up to May 31')
ylabel('Effective Repro. Number, R_e')
%legend({'Stay at Home Order(s) in Effect'},'FontSize',10)
set(gca,'FontSize',16)
load('Cfits.mat')
load('TCTfits.mat')
load('rhosRev.mat')
load('TetsRevision.mat')
for j = 1:55
